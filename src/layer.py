...
class QueueLayer(YowInterfaceLayer):
    ...
    EVENT_SEND_VIDEO = "org.openwhatsapp.yowsup.porp.queue.sendvideo"
    EVENT_SEND_AUDIO = "org.openwhatsapp.yowsup.porp.queue.sendaudio"

    def __init__(self, sendQueue):
        # print(YowInterfaceLayer.__dict__)
    ...
    def onEvent(self, layerEvent):
        ...
        if self.assertConnected():
            ...
            if layerEvent.getName() == self.__class__.EVENT_SEND_VIDEO:
                print(layerEvent.__dict__)
                path = layerEvent.getArg("path")
                number = layerEvent.getArg("number")
                caption = layerEvent.getArg("msg")
                jid = number
                # path = "/home/praveen/projects/mine/python/yowsup-stalker/sample.mp4"
                self.output("Trying to send Video %s " % path)
                entity = RequestUploadIqProtocolEntity(RequestUploadIqProtocolEntity.MEDIA_TYPE_VIDEO, filePath=path)
                successFn = lambda successEntity, originalEntity: self.onRequestUploadResult(jid, path, successEntity, originalEntity, caption)
                errorFn = lambda errorEntity, originalEntity: self.onRequestUploadError(jid)
                self._sendIq(entity, successFn, errorFn)
    ...
    def onRequestUploadResult(self, jid, filePath, resultRequestUploadIqProtocolEntity, requestUploadIqProtocolEntity, caption = None):
        ...
        if resultRequestUploadIqProtocolEntity.isDuplicate():
            if requestUploadIqProtocolEntity.mediaType == RequestUploadIqProtocolEntity.MEDIA_TYPE_VIDEO:
    ...
    def doSendVideo(self, filePath, url, to, ip = None):
        entity = VideoDownloadableMediaMessageProtocolEntity.fromFilePath(filePath, url, RequestUploadIqProtocolEntity.MEDIA_TYPE_VIDEO, ip, to)
        self.toLower(entity)
